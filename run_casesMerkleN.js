...
for (const [name, input] of Object.entries(cases)) {
  ...
  try {
    ...
    execSync(
      `node MerkleN_js/merkleN_js/generate_witness.js MerkleN_js/merkleN_js/merkleN.wasm input.json witness_${name}.wtns`,
      { stdio: "inherit" }
    );
    ...
    execSync(
      `snarkjs groth16 verify verification_key.json public_${name}.json proof_${name}.json`,
      { stdio: "inherit" }
    );
