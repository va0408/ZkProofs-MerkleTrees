...
# Fourth Question
The extensions I proposed—bucketing, q‑ary trees, and automated test input generation—vary in feasibility. Bucketing and q‑ary trees are possible in theory, but they require parametrized circuits and infrastructure that were not practical within the scope of this project, so they remain outside its reach. Automating test inputs is feasible and would clearly improve reproducibility and reduce manual overhead, but it was not prioritized due to time constraints. 

What I actually implemented were smaller circuits that could be reproduced in a didactic environment: Merkle trees of four and ten leaves, combined with a modular sum gadget. Invalid cases in Merkle4 correctly triggered assertion failures, which demonstrates soundness, and since MerkleN uses the same templates with only additional levels, the same rejection behavior applies. Completeness and soundness are therefore preserved across both circuits. The design does not include recursion or bucketing and relies on templates with a fixed number of leaves, so changing N requires generating a new version of the circuit, which limits flexibility. As a result, the system scales to dozens of leaves but not to millions, and proving grows linearly with the number of constraints.  Overall, this is a very toy implementation: it captures the core idea of Reckle+ Trees in an accessible and reproducible way, but without the advanced recursion or large‑scale optimizations of the original design.

# Fifth Question
The idea was to implement Merkle trees of different sizes and compare both the execution times and the sizes of the generated artifacts. I started by testing with a Merkle tree of 4 leaves, then one of 10, and I thought about extending it to a larger range, between 15-30 leaves. For each one I prepared test cases that included correct situations, invalid cases, and also edge scenarios. The intention was to upload everything to GitHub Actions and take advantage of CI to automate the verifications. However, very large trees require a powers of tau of considerable size, and the free GitHub server does not support that load. So I reduced the scope and parallelize the tasks, so that MerkleN (with ten leaves), suma_modular, and Merkle (with four leaves) could run at the same time.

The implementation was carried out using Circom and snarkJS. Circuits were defined for Merkle trees of fixed sizes, combined with a modular sum gadget so that the output digest included both the Merkle root and the Reduce result. Valid cases produced accepted proofs, while invalid ones failed at witness generation with an Assert Failed error. For MerkleN, only valid cases were tested due to time constraints, but the implementation is structurally identical to Merkle4, so the same rejection behavior applies.
...
Each implementation needed its own configuration and dependencies, and every time I switched from one to another I had to restart the processes, which led me to limit the number of trees and test cases. In addition, any modification in the implementation implied generating a new folder with the witnesses, verification key, and repeating the setup, which made adding cases tedious. In the end, the trees I was able to run are toy examples, and the differences between extreme inputs were not relevant: even with ten leaves and very large numbers, times and sizes did not vary much. That is why I tried to make the cases representative, even if they did not cover massive scenarios.
...
It would be interesting to experiment with larger trees, between 50 and 100 leaves, to observe how the proving and verification times scale, and useful to automate the generation of test cases, because doing it manually becomes repetitive and does not add much new information. In contrast with what is seen in the papers, where trees with millions of leaves are used, this project remained at a small scale, but it served to understand the practical limitations of running cryptographic proofs in a free CI environment and to explore how circuits behave in controlled scenarios.
...
# Conclusion
Even though the implementation was limited to small Merkle trees and did not include recursion, it showed the essential properties: constant proof size, stable verification cost, and rejection of invalid witnesses. The comparison with the paper highlights the difference between accessible prototypes and large‑scale research systems, and also shows that even very small models are useful to understand practical limitations and to explore how circuits behave under controlled conditions.
