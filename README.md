...
# Second Question 
The original paper uses Plonky2, a recursive SNARK system implemented in Rust. Plonky2 is transparent, meaning it doesn’t require a trusted setup, and it was designed to handle recursion efficiently, which means it can chain proofs together and update them without recomputing everything from scratch. Proof sizes stay fixed at around 110–120 KiB regardless of batch size, which makes scalability easier, and verification is very fast (3–6 ms), practical for blockchain applications like digest translation or BLS aggregation. The most relevant properties here are the native recursion support (efficiently updating proofs through recursive composition), the small and predictable proof size (always around the same compact size), and the low verifier cost (milliseconds per check).
