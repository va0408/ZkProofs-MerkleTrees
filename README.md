...
# Fifth Question
The idea was to implement Merkle trees of different sizes and compare both the execution times and the sizes of the generated artifacts. I started with a Merkle tree of 4 leaves, then one of 10, and considered extending it to 15–30 leaves. For each one I prepared test cases that included valid situations, invalid cases, and edge scenarios. The intention was to upload everything to GitHub Actions and take advantage of CI to automate the verifications. However, very large trees require a powers of tau of a size that the free GitHub server does not support. I reduced the scope and parallelized the tasks, so that MerkleN (with ten leaves), suma_modular, and Merkle (with four leaves) could run at the same time.
...
# Conclusion
In future implementations it would be interesting to experiment with larger trees, between 50 and 100 leaves, to observe how the proving and verification times scale, and useful to automate the generation of test cases. Even though this project remained at a small scale and did not include recursion, it still showed the essential properties—constant proof size, stable verification cost, and rejection of invalid witnesses—and highlighted the difference between accessible prototypes and large‑scale research systems.
